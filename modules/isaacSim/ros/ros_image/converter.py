...
def imgmsg_to_cvimage(msg: Image, color_space: str | None = None) -> Imagebytes:
...
def compressed_imgmsg_to_cvimage(
    msg: CompressedImage,
    ...
    """
    img: Imagebytes = cv2.imdecode(np.frombuffer(msg.data, np.uint8), cv2.IMREAD_UNCHANGED)
    if color_space:
        return convert_color(img, 'bgr8', color_space)
    return img


def cvimage_to_compressed_imgmsg(cvim, dst_format='jpeg', quality: int = 100):
    """
    Convert an OpenCV `cv::Mat` type to a ROS sensor_msgs::CompressedImage message.
    
    Args:
        cvim (np.ndarray): The input image (uint8 or uint16).
        dst_format (str): Desired output format ('jpg', 'png', etc.).
        quality (int): Compression quality (for JPEG only).
    
    Returns:
        CompressedImage: The resulting compressed image message.
    """
    ...
    try:
        if cvim.dtype == np.uint16 and dst_format.lower() in ['jpg', 'jpeg']:
            raise ValueError("JPEG does not support uint16. Use PNG or another format.")
        
        if dst_format.lower() in ['jpg', 'jpeg']:
            # Use JPEG compression for uint8
        ...
        cmprs_img_msg.data = buffer
    except RuntimeError as e:
        raise ImageConversionError(e)
    
    return cmprs_img_msg

def cvimage_to_imgmsg(cvim, header = None):
    
    def dtype_with_channels_to_cvtype2(dtype, n_channels):
        get_key = lambda dict, value: [k for k, v in dict.items() if v == value]
        return '%sC%d' % ( get_key(DEPTHMAP, dtype.name)[0], n_channels)
