...
class GroundingDINOLocal:
    ...
    def predict_shapes(self, image, image_path=None, text_prompt=None):
        ...
        #     _categorys.append(label)
        #     _socres.append(conf)

        # return {"boxes": _boxes, "categorys": _categorys, 'scores': _socres, 'masks': []}
        shapes = []
        scores = []
        for box, label_info in zip(boxes, pred_phrases):
            x1, y1, x2, y2 = box
            ...
            shape = Shape(
                label=str(label), shape_type=GroundingDINOLocal.default_output_mode
            )
        ...
        return {"scores": scores, "shapes": shapes}
