...
class MainWindow(QtWidgets.QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None, 
        ...
        if (frozen_path) :
            self.actions.beginner = (
                open_next_image, open_prev_image, verify, thumbnail, save, save_format, None, create, copy, delete, None,
            ...
            self.actions.beginner = (
                open, open_dir, change_save_dir, open_next_image, open_prev_image, verify, thumbnail, save, save_format, None, create, copy, delete, None,
                zoom, zoom_in, zoom_out, fit_window, fit_width)
        ...
        self.statusBar().showMessage('%s started.' % LABELGTING)
        ...
        if self.default_save_dir is None and save_dir is not None and os.path.exists(save_dir):
            ...
            self.statusBar().showMessage('%s started. Annotation will be saved to %s' %
                                         (LABELGTING, self.default_save_dir))
            self.statusBar().show()
    ...
    def setDebugLevel(self, level) :
        ...
        self.debug.debug("Change [ %s ] debug level." % LABELGTING )
    ...
    def combo_selection_changed(self, index):
        ...
        items = self.label_list.findItems(text, Qt.MatchFixedString | Qt.MatchCaseSensitive)
        if(items):
            self.combo_box.current_item()
    ...
    def change_save_dir_dialog(self, _value=False):
        ...
        dir_path = ustr(QtWidgets.QFileDialog.getExistingDirectory(self,
                                                         '%s - Save annotations to the directory' % LABELGTING, path,  QtWidgets.QFileDialog.ShowDirsOnly
    ...
    def open_annotation_dialog(self, _value=False):
        ...
        if self.label_file_format == LabelFileFormat.PASCAL_VOC:
            ...
            filename = ustr(QtWidgets.QFileDialog.getOpenFileName(self, '%s - Choose a xml file' % LABELGTING, path, filters))
            if filename:
    ...
    def open_dir_dialog(self, _value=False, dir_path=None, silent=False):
        ...
        if silent != True:
            target_dir_path = ustr(QtWidgets.QFileDialog.getExistingDirectory(self,
                                                                    '%s - Open Directory' % LABELGTING, default_open_dir_path,
    ...
    def open_file(self, _value=False):
        ...
        filename = QtWidgets.QFileDialog.getOpenFileName(self, '%s - Choose Image or Label file' % LABELGTING, path, filters)
        if filename:
            if isinstance(filename, (tuple, list)):
                filename = filename[0]
            self.cur_img_idx = 0
    ...
    def save_file_dialog(self, remove_ext=True):
        caption = '%s - Choose File' % LABELGTING
...
def inverted(color):
    return QtGui.QtGui.QColor(*[255 - v for v in color.getRgb()])
